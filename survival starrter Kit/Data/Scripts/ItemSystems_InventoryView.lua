...
local INVENTORY_LOADOUTOUTSIDE = script:GetCustomProperty("OutsideInventoryLoadout"):WaitForObject()
...
function view:Init()
    ...
    self:Close()
    self:MoveLoadoutOutOfInventory()
end
...
function view:InitLoadoutSlots()
    self.allSlots = self.allSlots or {}
    inventory:SetEquipedLoadout(nil)
    self.loadoutSlots = {}
    TraverseAndSetupSlots(PANEL_LOADOUTS, function(slot) table.insert(self.loadoutSlots, slot) end)
    for i,slot in ipairs(self.loadoutSlots) do
        slot.clientUserData.isLoadout = true
        slot.clientUserData.slotIndex = inventory:ConvertLoadoutSlotIndex(i)
        -- If the slot has a UseKey property then when we use the 
        if slot:GetCustomProperty("UseKey") then
            local useKey = slot:GetCustomProperty("UseKey")
            LOCAL_PLAYER.bindingPressedEvent:Connect(function(_,key) 
                local slotIndex = slot.clientUserData.slotIndex
                local item = inventory:GetItem(slotIndex)
                if item and useKey == key and inventory:GetEquipedLoadout() ~= slotIndex and
                 not self:IsLoadoutOnCooldown() and not LOCAL_PLAYER.isDead then
                    self:ApplyLoadoutCooldowns()
                    if item:HasConsumptionEffect() then
                        inventory:ConsumeItem(slotIndex)
                    elseif item:IsEquippable() then
                        inventory:SetEquipedLoadout( slotIndex) 
                        self:HighlightSlot(slot)
                        inventory:EquipSlotItem(slotIndex)
                    end
                    PlaySound(ItemThemes.GetItemSFX(item:GetType()))
                    Events.Broadcast("InventoryView.EquippedItem", item, slotIndex )
                elseif useKey == key and inventory:GetEquipedLoadout() ~= slotIndex and
                    not self:IsLoadoutOnCooldown() and not LOCAL_PLAYER.isDead  then
                    self:ApplyLoadoutCooldowns()
                    self:HighlightSlot(slot)
                    inventory:UnEquipSlotItem(slotIndex)
                    PlaySound(SFX_EQUIP)
                end
            end)
        end
...
function view:AttemptMoveItem(fromSlotIndex, toSlotIndex)
    if inventory:CanMoveItem(fromSlotIndex, toSlotIndex) then
        ...
        local swapItem = inventory:GetItem(toSlotIndex)
        -- Don't allow items not marked as loadout to enter the loadout slots
        if movingItem and movingItem:GetEquipSlotType() ~= "Loadout" and inventory:IsLoadoutSlot(toSlotIndex) then
            PlaySound(SFX_DENIED)
            return
        end
        if swapItem then
            if swapItem:GetEquipSlotType() ~= "Loadout" and inventory:IsLoadoutSlot(fromSlotIndex) then
                PlaySound(SFX_DENIED)
                return
            end
        end

        if self.isCursorInContainer then
            local item = inventory:GetItem(fromSlotIndex)
            PlaySound(ItemThemes.GetItemSFX(item:GetType()))
            inventory:MoveItem(fromSlotIndex, 0) -- Trash the item when moved from inventory into container
            Events.Broadcast("InventoryView.MovedItem",fromSlotIndex, 0 )
        else
            if self.fromSlotIndex ~= toSlotIndex then
                inventory:MoveItem(fromSlotIndex, toSlotIndex) -- Move normally
...
function view:PerformClickAction()
    ...
    if clickedItem:IsEquippable() and not clickedItem:IsBackpack() and not clickedItem:HasConsumptionEffect() and not self.isCursorInContainer then
        -- Equippable item.
...
function view:PerformDragDropAction()
    if self.slotUnderCursor or not self.isCursorInBounds and not self.isHoveringOnSalvage then
        ...
        if inventory:GetEquipedLoadout() and (self.fromSlotIndex == inventory:GetEquipedLoadout() or toSlotIndex == inventory:GetEquipedLoadout()) then
            self:StopHighlightSlots()
            inventory:SetEquipedLoadout(nil)
            inventory:UnEquipSlotItem() -- If the player moves the item they currently have equipped.
        end
...
function view:MoveLoadoutOutOfInventory()
    PANEL_LOADOUTS.parent = INVENTORY_LOADOUTOUTSIDE
    PANEL_ITEM_HOVER.parent = INVENTORY_LOADOUTOUTSIDE
end

function view:MoveLoadoutIntoInventory()
    PANEL_LOADOUTS.parent = INVENTORY_VIEW
    PANEL_ITEM_HOVER.parent = INVENTORY_VIEW
end
...
function view:DrawSlots()
    for _,slot in ipairs(self.allSlots) do
        ...
        if item and not isHeldSlot then
            ...
            if inventory:IsLoadoutSlot(slot.clientUserData.slotIndex) or inventory:IsBackpackSlot(slot.clientUserData.slotIndex) then
