...
local MAX_DISTANCE_SQUARED = 300^2 -- How far we can be to loot items
...
local function OnLootAddedToFolder(_,item)
    Task.Wait(0.2) -- I guess we have to wait for the object to move into position...
    lootRegions.AddItemToRegion(item)
end
...
function Tick()
    ...
    -- If the player moves from one location to another then unregister all loot at last location
    if lastPos then
        if (lastPos - playerPos).sizeSquared > MAX_DISTANCE_SQUARED then
            for x=1,SEARCHSIZE do
                for y=1,SEARCHSIZE do
                    local lootPickups = lootRegions.GetLootsAtLocation(Vector2.New(lastPos.x + (x-SEARCHSIZE/2-0.5) * SIZE,lastPos.y + (y-SEARCHSIZE/2-0.5) * SIZE))
            ...
            if #lootPickups > 0 then
                for _, lootDrop in pairs(lootPickups) do
                    if Object.IsValid(lootDrop) then
                        ...
                        if lootInfo ~= "" and (lootDrop:GetWorldPosition() - playerPos).sizeSquared < MAX_DISTANCE_SQUARED then
