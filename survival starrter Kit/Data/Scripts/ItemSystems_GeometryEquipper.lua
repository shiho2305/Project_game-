--[[
    ...
    at a specificed socket. The determined socket is derived from a custom property on the item in registered items.
]]
...
-- Assign the owner of the geometry equipper.
local OWNER = nil
while not OWNER do
    Task.Wait()
...
-- Wait until the inventory loads
...
-- Destroy geometry from a equipment slot
-- @param int slotIndex
...
-- Creates geometry on the character anytime a slot is changed
-- @param slotIndex int
-- @param Item item
...
---------------------------------------------------------------------------------------------------------
-- Iterates through the equipment slots and display the the geometry for the equipment.
...
-- Clean up the geometry when the geometry equipper is destroyed.
