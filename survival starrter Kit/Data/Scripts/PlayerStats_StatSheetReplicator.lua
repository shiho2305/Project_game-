...
local function ClientSetupReplicatedStatSheet()
    ...
    local statSheetUpdateTask
    statSheetUpdateTask = Task.Spawn(function() 
        if not Object.IsValid(OWNER) then statSheetUpdateTask:Cancel() return end
        ClientUpdateStatSheet(OWNER.clientUserData.statSheet, statModifiers)
    end)
