...
function Inventory.New(database, owner, backpackSize, loadoutSize, equipSlots)
    ...
    o.BACKPACK_CAPACITY = backpackSize or 16
    o.LOADOUT_CAPACITY = loadoutSize or 4
    o.TOTAL_CAPACITY = #o.EQUIP_SLOTS + o.BACKPACK_CAPACITY + o.LOADOUT_CAPACITY
    o.LOADOUT_SLOT = nil
...
function Inventory:ConvertBackpackSlotIndex(backpackIndex) -- int backpackIndex
...
function Inventory:ConvertEquipSlotIndex(slotType, slotNumber) -- string slotType, int slotNumber
...
-- Converts the 1-based loadout index into the correct inventory slot index.
function Inventory:ConvertLoadoutSlotIndex(loadoutIndex) -- int loadoutIndex
    assert(1 <= loadoutIndex and loadoutIndex <= self.BACKPACK_CAPACITY + #self.EQUIP_SLOTS)
    return #self.EQUIP_SLOTS + self.BACKPACK_CAPACITY + loadoutIndex
end

-- Returns True if the slot represents a backpack item.
function Inventory:IsBackpackSlot(slotIndex) -- int slotIndex
    return slotIndex and #self.EQUIP_SLOTS < slotIndex and slotIndex <= self.TOTAL_CAPACITY - self.LOADOUT_CAPACITY
end

-- Returns True if the slot represents an equipment item. This being your helmet, chest piece, belt, and boots.
function Inventory:IsEquipSlot(slotIndex) -- int slotIndex
    return slotIndex and 1 <= slotIndex and slotIndex <= #self.EQUIP_SLOTS
end

-- Returns True if the slot represents a loadout slot.
function Inventory:IsLoadoutSlot(slotIndex) -- int slotIndex
    return slotIndex and slotIndex > self.TOTAL_CAPACITY - self.LOADOUT_CAPACITY
end

-- Returns True if the slot accepts the given type.
function Inventory:IsEquipSlotType(slotIndex, slotType)  -- int slotIndex, string slotType
    return self:IsEquipSlot(slotIndex) and self.EQUIP_SLOTS[slotIndex].slotType == slotType
end

-- Returns True if the slot is empty.
function Inventory:IsEmptySlot(slotIndex) -- int slotIndex
...
-- Get equipped equiped slot
function Inventory:GetEquipedLoadout()
    return self.LOADOUT_SLOT
end

-- Set equipped loadout slot
function Inventory:SetEquipedLoadout(valueSlot)  -- int valueSlot
    self.LOADOUT_SLOT = valueSlot
end
...
-- Gets the first free Loadout slot and returns the slot index.
function Inventory:GetFreeLoadoutSlot()
    for slotIndex =  #self.EQUIP_SLOTS + self.BACKPACK_CAPACITY, self.TOTAL_CAPACITY do
        if self:IsLoadoutSlot (slotIndex) and self:IsEmptySlot(slotIndex) then
            return slotIndex
        end
    end
end
...
function Inventory:CanClaimLootIgnoreOverflow(lootID) -- int lootID
    ...
    if lootInfo and not lootInfo.isClaimed then
        ...
        elseif (lootInfo.item:GetEquipSlotType() == "Loadout") then
            return (not self:IsBackpackFull()) or (not self:GetFreeLoadoutSlot())
        else
            return self:GetFreeBackpackSlot() ~= nil
        end
    end
...
-- Force unequip whatever the characer is holding in a given slot
function Inventory:UnEquipSlotIndex(index)
    if self:GetEquipedLoadout() == index then
        self:UnEquipSlotItem()
    end
end
...
function Inventory:_CanMoveItemOneWay(fromSlotIndex, toSlotIndex)
    ...
    if self:IsLoadoutSlot(toSlotIndex) then
        return true
    end
...
function Inventory:_CanAccommodateItem(item)
   if self:GetFreeBackpackSlot() or
        ...
        -- TODO: Implement below along with stackables in loadout slots.
        --or (self:GetFreeLoadoutSlot() and item:GetEquipSlotType() == "Loadout" ))
...
function Inventory:_TryToEquipItem(item)
    if self:GetFreeLoadoutSlot() and item:GetEquipSlotType() == "Loadout" then
        local loadoutSlot = self:GetFreeLoadoutSlot()
        self:_SetSlotItem(loadoutSlot, item)
        if(self:GetEquipedLoadout() == nil and item:GetType() ~= "Consumable") then
            Task.Spawn(function() self:EquipSlotItem(loadoutSlot) end, .3)
        end
    else
        local slotIndex = self:GetFreeBackpackSlot()
        self:_SetSlotItem(slotIndex, item)
