...
local function UpdatePlayerEntry(player)
	...
	local playerNameText, teamColorImage, playerKillsText, playerDeathsText =
		entry:GetCustomProperty("PlayerName"):WaitForObject(),
		entry:GetCustomProperty("TeamColor"):WaitForObject(),
		entry:GetCustomProperty("Kills"):WaitForObject(),
		entry:GetCustomProperty("Deaths"):WaitForObject()

		playerKillsText.text = tostring(player.kills)
		playerDeathsText.text = tostring(player.deaths)
